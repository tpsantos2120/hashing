...
/**
 * Demonstrates API key distribution and JWT signing/verification using HS256 algorithm.
 */
public class HS256ApiKeyDemo {

  /**
   * Demonstrates the complete API key distribution flow for HS256.
   *
  ...
  public static void demonstrate(String subject, String issuer, Date issuedAt, Date expiration) {
    System.out.println("1. HS256 (HMAC-SHA256) - API Key Distribution Demonstration");
    System.out.println("=".repeat(80));
    System.out.println();

    // Server: Generate a secret key (256 bits minimum for HS256)
    SecretKey secretKey = SecretKeyGenerator.generateHS256Key();
    ...
    System.out.println("Algorithm: HS256 (HMAC-SHA256)");
    System.out.println();
  ...
  /**
   * Demonstrates the end user flow: receiving encoded key, decoding, signing JWT, and server verification.
   */
  private static void demonstrateEndUserFlow(String encodingType, String encodedKey,
    ...
    SecretKey reconstructedKey = new SecretKeySpec(decodedKeyBytes, "HmacSHA256");
